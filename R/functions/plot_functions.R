plot_performance_model <- function(performance_model, performance_data) {
  ...
  preds_plot <- preds |>
    ...
    #            aes(x = time_hours, y = yi, size = size), alpha = 0.1) +
...
# 
# plot_perceptual_model <- function(perceptual_model, perceptual_data) {
#   
#   preds <- predictions(perceptual_model, 
#                        re_formula= NA,
#                        newdata = perceptual_model$data
#                        
#   ) |>
#     mutate(
#       recovery_mode = factor(recovery_mode,
...
#       axis.title = element_text(size = 10)
#     ) +
#     scale_y_continuous(limits = c(-3.5,3.5), breaks = seq(-5, 5, by = 1)) +
#     scale_x_continuous(limits = c(0,120), breaks = seq(0,120, by = 24))
#   
#   
#   contr_ref <- comparisons(
